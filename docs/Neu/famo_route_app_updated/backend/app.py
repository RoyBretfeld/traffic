...
# Global variables to hold last processed customers and tours. These are
# used to serve downloadable route files after a user uploads a CSV.
last_customers = None
last_tours = None
...
def create_app():
    ...
    def upload_file():
        ...
        if file and allowed_file(file.filename):
            ...
            # Store the last processed data in global variables for download
            global last_customers, last_tours
            last_customers = customers
            last_tours = tours
            # Build response structure with statistics
            resp = {
                ...
                "stats": {
                    ...
                    "total_time_min": 0.0,
                    "avg_stops_per_tour": 0.0,
                },
            }
            total_stops = 0
            for idx, tour in enumerate(tours):
                stats = util.compute_route_stats(
                    FAMO_COORDS, customers, tour, speed_kmh=50.0, service_time_min=2.0
                )
                total_stops += stats["num_stops"]
                # accumulate global stats
                resp["stats"]["total_distance_km"] += stats["total_distance_km"]
                resp["stats"]["total_travel_time_min"] += stats["travel_time_min"]
                resp["stats"]["total_service_time_min"] += stats["service_time_min"]
                resp["stats"]["total_time_min"] += stats["total_time_min"]
                tour_data = {
                    "tour_index": idx + 1,
                    "customers": [],
                    "stats": stats,
            ...
            if resp["num_tours"] > 0:
                resp["stats"]["avg_stops_per_tour"] = total_stops / resp["num_tours"]
            else:
                resp["stats"]["avg_stops_per_tour"] = 0.0
            return jsonify(resp)
        else:
            return jsonify({"error": "Unsupported file type"}), 400

    @app.route("/download_route/<int:tour_index>", methods=["GET"])
    def download_route(tour_index: int):
        """Download a CSV file containing the specified tour.

        The `tour_index` parameter is 1-based, corresponding to the index
        assigned in the JSON response after upload. If no upload has
        ...
        global last_customers, last_tours
        if last_customers is None or last_tours is None:
            return jsonify({"error": "No route data available. Please upload a file first."}), 400
        idx = tour_index - 1
        if idx < 0 or idx >= len(last_tours):
            return abort(404)
        route_indices = last_tours[idx]
        ...
        writer.writerow([
            "order", "kdnr", "name", "street", "plz", "city", "lat", "lon", "bar",
        ])
        for pos, cust_idx in enumerate(route_indices):
            cust = last_customers[cust_idx]
            writer.writerow([
                pos + 1,
                cust["kdnr"],
                cust["name"],
                ...
                cust["lat"],
                cust["lon"],
                "yes" if cust["bar"] else "no",
            ])
        output.seek(0)
        from flask import Response
        filename = f"route_{tour_index}.csv"
        return Response(
            output.getvalue(),
            mimetype="text/csv",
