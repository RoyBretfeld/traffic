...
def test_manual_geo_api(tmp_path, monkeypatch):
    ...
    # Verifizieren dass in DB gespeichert
    with ENGINE.begin() as c:
        row = c.execute(text(
            "SELECT address_norm, lat, lon, source, by_user FROM geo_cache WHERE address_norm=:a"
        ), {"a": test_address}).mappings().first()
        
        assert row is not None
        assert row["address_norm"] == test_address
        assert abs(row["lat"] - test_lat) < 1e-6
        assert abs(row["lon"] - test_lon) < 1e-6
        assert row["source"] == "manual"
        assert row["by_user"] == "test_user"
