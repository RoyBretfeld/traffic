...
def _geocode_with_geoapify(address: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if exc.response.status_code == 401:
            raise HTTPException(status_code=401, detail="Geocoding API Key fehlt oder ist ungültig")
        elif exc.response.status_code == 402:
            raise HTTPException(status_code=429, detail="Geocoding API Quota überschritten")
        elif exc.response.status_code == 403:
            raise HTTPException(status_code=403, detail="Geocoding API Zugriff verweigert")
