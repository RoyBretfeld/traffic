...
class AIOptimizer:
    ...
    async def optimize_route(
        ...
        # Prüfe Constraints
        if len(stops) > rules.max_stops_per_tour:
            raise ValueError(
                f"Zu viele Stopps ({len(stops)}). Maximum: {rules.max_stops_per_tour}"
            )
...
Zeit-Constraints:
- Max. {rules.max_driving_time_to_last_customer} Minuten Fahrzeit bis zum letzten Kunden
- {rules.service_time_per_customer_minutes} Minuten Verweilzeit pro Kunde
- {rules.return_trip_buffer_minutes} Minuten Puffer für Rückfahrt zum Depot
- Start/Ziel: {rules.start_location} (Koordinaten: {rules.depot_lat}, {rules.depot_lon})
...
- Max. {rules.max_stops_per_tour} Kunden pro Tour
