...
async def do_match(file_path: str):
    ...
    if ENFORCE:
        ...
        if missing:
            ...
            # Format: [{"address": str, "result": {"lat": str, "lon": str, ...}, "status": "ok"}, ...]
            for item in filled:
                # Überspringe Meta-Einträge
                if "_meta" in item:
                    continue
                if item.get("status") == "ok" and "result" in item:
                    addr = item["address"]
                    result = item["result"]
                    # Konvertiere String-Koordinaten zu Float und erstelle kompatibles Format
                    try:
                        ...
                        geo[addr] = {
                            "lat": lat,
                            "lon": lon,
                            "source": result.get("_note", "geocoded"),
                            "src": "geocoded",
                            "precision": None,
                            "region_ok": None,
                        }
    ...
    for i, customer in enumerate(tour_data["customers"]):
        ...
        # Status zum DTO hinzufügen (wird für Zusammenfassung benötigt)
        extra['status'] = status
