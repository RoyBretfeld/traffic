...
from repositories.geo_fail_repo import (
    clear, clear_all, get_fail_status, get_fail_stats, get_fail_reasons, cleanup_expired
)
...
@router.post("/api/geocode/clear-all-fail-cache")
def clear_all_fail_cache():
    """
    Löscht ALLE Einträge aus dem Fail-Cache.
    
    WICHTIG: Verwendung wenn die Erkennungsroutine überarbeitet/verbessert wurde.
    Alle Adressen werden dann erneut versucht, um 100% Abdeckung zu erreichen.
    
    - Löscht alle Einträge (aktiv und abgelaufen)
    - Ermöglicht Retry für alle bisher fehlgeschlagenen Adressen
    - Ziel: 100% Abdeckung durch wiederholtes Versuchen
    """
    try:
        deleted_count = clear_all()
        stats = get_fail_stats()
        ...
        return JSONResponse({
            ...
            "remaining_stats": stats,
            "message": f"{deleted_count} Einträge gelöscht - alle Adressen werden erneut versucht"
        })
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Fehler beim Löschen des Fail-Cache: {str(e)}")
