...
async def api_tourplan_match(file: str = Query(..., description="Pfad zur CSV-Datei (absolut oder relativ)")):
    """
    ...
    try:
        ...
        print(f"[MATCH DEBUG] Original file parameter: {file}")
        ...
        if not os.path.exists(file_path_str) and not p.exists():
            ...
            if not existing_path:
                ...
                if staging_file.exists():
                    ...
                    raise HTTPException(404, detail=f"Datei nicht gefunden: {file_path_str} (original: {file}, decoded: {decoded_file}, filename: {filename_only})")
        ...
        try:
            ...
            import traceback
            error_trace = traceback.format_exc()
            print(f"[MATCH ERROR] Parser-Fehler für {p}: {parse_error}")
        ...
        for i, customer in enumerate(tour_data["customers"]):
            ...
            customer_name = customer.get('name', '')
            synonym_hit = None
            canon = None  # Alias-Kanonisierung (wird später gesetzt)
            ...
            extra = {
                "row": int(i + 1),
                "alias_of": canon if not synonym_hit else None,  # zeigt, wenn Alias greift
                "markers": marks,
                "status": status,
                "manual_needed": manual_is_open(addr_norm) if not synonym_hit else False,
            }
        ...
        # HTTPException wird direkt weitergegeben (hat bereits JSON-Format)
        raise
        ...
        print(f"[MATCH ERROR] Traceback:\n{error_details}")
        # Detaillierte Fehlermeldung für Debugging (als JSON zurückgeben)
        error_msg = f"Match-Fehler für '{file}': {str(e)}"
        return JSONResponse(
            {"detail": error_msg, "error": str(e), "file": file},
            status_code=500,
