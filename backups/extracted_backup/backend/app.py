...
from routes.workflow_api import router as workflow_api_router
...
from routes.summary_api import router as summary_api_router
from routes.test_dashboard_api import router as test_dashboard_router
from routes.address_recognition_api import router as address_recognition_router
from routes.endpoint_flow_api import router as endpoint_flow_router
from routes.ai_test_api import router as ai_test_router
from routes.backup_api import router as backup_api_router
from routes.engine_api import router as engine_api_router
from routes.coordinate_verify_api import router as coordinate_verify_router
...
def create_app():
    ...
    app.include_router(address_recognition_router)
    app.include_router(endpoint_flow_router)
    app.include_router(ai_test_router)
    app.include_router(backup_api_router)
    app.include_router(engine_api_router)
    app.include_router(coordinate_verify_router)
...
async def tourplan_visual_test(file: UploadFile = File(...)) -> JSONResponse:
    ...
    try:
        ...
        print(f"[VISUAL-TEST] Datei gespeichert: {tmp_file_path} ({len(content)} bytes)")
...
@app.get("/ui/coordinate-verify", response_class=HTMLResponse, tags=["ui"], summary="Koordinaten-Verifizierung")
async def coordinate_verify_page():
    """Zeigt die Koordinaten-Verifizierungs-Seite an."""
    try:
        with open("frontend/coordinate-verify.html", "r", encoding="utf-8") as f:
            content = f.read()
        return HTMLResponse(content=content, media_type="text/html; charset=utf-8")
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="Koordinaten-Verifizierungs-Seite nicht gefunden")
...
async def ai_test_page():
    ...
    try:
        with open("frontend/ai-test.html", "r", encoding="utf-8") as f:
            content = f.read()
...
async def test_dashboard():
    ...
    try:
        ...
        with open("frontend/test-dashboard.html", "r", encoding="utf-8") as f:
            content = f.read()
