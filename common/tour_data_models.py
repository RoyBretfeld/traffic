...
class TourInfo:
    ...
    def get_base_name(header: str) -> str:
        ...
        Entfernt "BAR", "Tour", "Uhr" am Ende für Gruppierung von W-Touren.
        Behält ANLIEF-Touren vollständig (z.B. "Anlief. T5, 9.30 Uhr" bleibt "Anlief. T5, 9.30").
        """
        name = _fix_broken_chars(header)
        
        # WICHTIG: Für W-Touren: "W-07.00 Uhr Tour" und "W-07.00 Uhr BAR" → beide zu "W-07.00"
        # Für ANLIEF-Touren: Behalte vollständig (haben keine BAR-Varianten)
        
        # Entferne "BAR" nur am Ende (z.B. "W-07.00 Uhr BAR" → "W-07.00 Uhr")
        name = re.sub(r'\s+BAR\s*$', '', name, flags=re.IGNORECASE)
        
        # Entferne "Tour" nur am Ende (z.B. "W-07.00 Uhr Tour" → "W-07.00 Uhr")
        name = re.sub(r'\s+Tour\s*$', '', name, flags=re.IGNORECASE)
        
        # Entferne "Uhr" am Ende (z.B. "W-07.00 Uhr" → "W-07.00", "9.30 Uhr" → "9.30")
        name = re.sub(r'\s+Uhr\s*$', '', name, flags=re.IGNORECASE)
